...
console.log("Connecting to SIM900R module");
var gprs = require('@amperka/gprs').connect(Serial3, P5, function(err) {
  if (err) throw err;
  gprs.connect('internet.beeline.ru', 'beeline', 'beeline', function(err) {
    if (err) throw err;
...
var gprsFuncs = {
  ...
  "init": function(callback) {
    ...
    var cb = function(r) {
      switch (s) {
        case 0:
          if (r === 'IIIIATE0' || r === 'IIII' + String.fromCharCode(255) + 'ATE0' || r === 'ATE0') {
            return cb;
          } else if (r === 'OK') {
            s = 1;
            at.cmd('AT+CPIN?\r\n', 500, cb);
  ...
  "powerOn": function(callback) {
    if (!digitalRead(_statusPin)) {
      digitalPulse(_powerPin, 1, 1000);
      setTimeout(function() {
        print('gprsFuncs.init(callback);');
...
exports.connect = function(options, connectedCallback) {

  options = options || {};

  _powerPin = options.powerPin || P2;
  _statusPin = options.statusPin || P3;
  _serial = options.port || Serial3;
  _serial.setup(options.serialSpeed || 9600);

  gprsFuncs.at = at = require('AT').connect(_serial);
