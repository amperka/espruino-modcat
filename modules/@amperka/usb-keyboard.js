...
var type = function(txt, sendSpeed, callback) {
    ...
    var code;
    var timeForWait
    if (typeof(sendSpeed) === 'number' && sendSpeed < 50) {
...
var changePressedKeys = function(opts) {
    ...
    // Protect from undefined
    opts = opts || 0;
    console.log('compare ' + pressedKeys + ' ' + opts);
    if (pressedKeys[0] !== modifiers || pressedKeys[2] !== opts &&
        pressedKeys[3] !== opts && pressedKeys[4] !== opts &&
        pressedKeys[5] !== opts && pressedKeys[6] !== opts &&
        pressedKeys[7] !== opts) {
        if (pressedKeys[0] !== modifiers) {
            pressedKeys[0] = modifiers;
        }
        console.log('compared keys ' + pressedKeys);
        for (i = 2; i < 8; i++) {
            console.log('changeKeys ' + pressedKeys);
        ...
        if (i === 8) {
            console.log('error ' + pressedKeys);
            return 1;
        }
        console.log('sendToHID ' + pressedKeys);
        E.sendUSBHID(pressedKeys);
    }
