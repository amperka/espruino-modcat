...
var WIFI_CONNECT_STATUS = ['WIFI DISCONNECT','WIFI CONNECTED','WIFI GOT IP','+CWJAP:1'];
...
var wifiFuncs = {
  ...
  'reset': function(callback) {
    at.cmd('\r\nAT+RST\r\n', 10000, function cb(d) {
      ...
      if (d == 'ready' || d == 'Ready.') {
        if (d == 'ready') {
          print('firmvare version 0.25');
        }
        if (d == 'Ready.') {
          print('firmvare version 0.50');
        }
        setTimeout(function() {
          wifiFuncs.init(callback);
        }, 1000);
      } else if (d === undefined) {
    ...
    at.cmd('AT+CWMODE=1\r\n', 1000, function(cwm) {
      if (cwm != 'no change' && cwm != 'OK') {
        ...
        at.cmd('AT+CWJAP='+JSON.stringify(ssid)+','+JSON.stringify(key)+'\r\n', 20000, function cb(d) {
          if (WIFI_CONNECT_STATUS.indexOf(d) >= 0) {
            return cb;
          }
          if (d != 'OK') {
  ...
  'createAP': function(ssid, key, channel, enc, callback) {
    at.cmd('AT+CWMODE=2\r\n', 1000, function(cwm) {
      ...
      if (encn < 0) {
        ...
        at.cmd('AT+CWSAP='+JSON.stringify(ssid)+','+JSON.stringify(key)+','+channel+','+encn+'\r\n', 5000,
                function(cwm) {
                  if (cwm != 'OK') {
